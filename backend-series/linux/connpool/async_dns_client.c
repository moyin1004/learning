...
void *dns_async_callback(void *arg) {
    ...
    printf("dns_async_callback start\n");
    while (1) {
...
int dns_async_client_destroy(struct async_context *ctx) {
    close(ctx->epfd);
    return pthread_cancel(ctx->threadid);
}
...
void dns_handle(struct dns_item *arg, int count) {
    for (int i = 0;i < count;i ++) {
		printf("name:%s, ip:%s\n", arg[i].domain, arg[i].ip);
	}
    printf("--------------\n\n");
}
...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; i < count; i++) {
        dns_async_client_commit(ctx, domain[i], dns_handle);
    }
    getchar();
